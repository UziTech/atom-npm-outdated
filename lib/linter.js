...
export class Linter {
  ...
  isLatestRange(version) {
    return ["x", "*", "latest"].includes(version.toLowerCase());
  }
  ...
  reportDependency(dependency, content, textEditor) {
    ...
    const prefix = this.getVersionPrefix(dependency.version);
    ...
    const validVersion = semver.valid(dependency.version);
    const isLatestVersionRange = this.isLatestRange(dependency.version);
    if (latest && (semver.validRange(dependency.version) || isLatestVersionRange) && (!validVersion || dependency.availableVersions.find((version) => version === validVersion))) {
      if (!isLatestVersionRange && !semver.satisfies(latest, dependency.version)) {
        ...
        if (!semver.satisfies(previousVersion, dependency.version) || isLatestVersionRange) {
      ...
      severity = 'error';
      text = `The package ${dependency.name} has an invalid ${validVersion ? '' : 'range '}version`;
    }
