...
export class Linter {
  ...
  reportDependency(dependency, content, textEditor) {
    ...
    if (latest && semver.validRange(dependency.version) && (!validVersion || dependency.availableVersions.find((version) => version === validVersion))) {
      if (!semver.satisfies(latest, dependency.version)) {
        ...
        if (!semver.satisfies(previousVersion, dependency.version) || this.isLatestRange(dependency.version)) {
